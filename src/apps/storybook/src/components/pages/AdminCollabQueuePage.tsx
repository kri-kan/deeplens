...
export function AdminCollabQueuePage({
  queueItems: initialQueueItems = DEFAULT_COLLAB_POSTS.filter((p) => p.curationStatus === 'queued'),
  ...
  const [queueList, setQueueList] = useState<CollabPostItem[]>(initialQueueItems);
  ...
  const handleSaveEdit = (postId: string, newTargets: string[]) => {
    setQueueList((prev) =>
      prev.map((p) => (p.id === postId ? { ...p, targetCollabAccounts: newTargets } : p))
    );
    onUpdateTargets?.(postId, newTargets);
    handleCloseEdit();
  };

  const handleUnqueue = (postId: string) => {
    setQueueList((prev) => prev.filter((p) => p.id !== postId));
    onUnqueueItem?.(postId);
  };
            ...
            <XStack alignItems="center" gap={10}>
              ...
              <YStack flex={1}>
                ...
                <Text fontSize={11} color="#6B21A8" lineHeight={15}>
                  {queueList.length} post(s) ready. Scripts will invite collaborators and auto-switch profiles to accept.
                </Text>
              </YStack>
            </XStack>
          </View>
