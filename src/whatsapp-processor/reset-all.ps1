...
Write-Host "1️⃣  Stopping application..." -ForegroundColor Cyan
try {
    taskkill /F /IM node.exe /T 2>$null
    Write-Host "   ✅ Application stopped" -ForegroundColor Green
} catch {
    Write-Host "   ℹ️  No running application found" -ForegroundColor Gray
}
...
# 2. Clean database
Write-Host ""
Write-Host "2️⃣  Cleaning database..." -ForegroundColor Cyan
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "   ✅ Database tables truncated" -ForegroundColor Green
    } else {
        Write-Host "   ⚠️  Database truncate failed: $result" -ForegroundColor Yellow
    }
} catch {
...
Write-Host ""
Write-Host "3️⃣  Cleaning MinIO bucket..." -ForegroundColor Cyan
...
try {
    ...
    if ($LASTEXITCODE -eq 0 -and $objects) {
        Write-Host "   🗑️  Removing objects..." -ForegroundColor Gray
        $removeResult = podman exec deeplens-minio mc rm --recursive --force local/$bucketName/ 2>&1
        
        if ($LASTEXITCODE -eq 0) {
            Write-Host "   ✅ MinIO bucket cleaned" -ForegroundColor Green
        } else {
            Write-Host "   ⚠️  Some objects may not have been removed" -ForegroundColor Yellow
        }
    } else {
        Write-Host "   ℹ️  Bucket is already empty or inaccessible" -ForegroundColor Gray
    }
} catch {
    Write-Host "   ⚠️  MinIO cleanup skipped: $_" -ForegroundColor Yellow
}

# 4. Clean WhatsApp session (optional)
if ($IncludeSession) {
    Write-Host ""
    Write-Host "4️⃣  Cleaning WhatsApp session..." -ForegroundColor Cyan
    
    $sessionPath = "./sessions/default_session"
    if (Test-Path $sessionPath) {
        try {
            Remove-Item -Path $sessionPath -Recurse -Force
            Write-Host "   ✅ Session deleted (QR code required on next start)" -ForegroundColor Green
        } catch {
            Write-Host "   ❌ Session cleanup failed: $_" -ForegroundColor Red
        }
    } else {
        Write-Host "   ℹ️  No session found" -ForegroundColor Gray
    }
}
