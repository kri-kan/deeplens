...
export default function ChatsAdminPage() {
    ...
    const includedItems = chats.filter(item => !item.isExcluded);
    const excludedItems = chats.filter(item => item.isExcluded);
        ...
        <Stack tokens={{ childrenGap: 24 }}>
            <Stack className={styles.highlightCard}>
                <Stack horizontal tokens={{ childrenGap: 16 }} verticalAlign="center">
                    <Icon
                        iconName="People"
                        styles={{ root: { fontSize: 48, color: tokens.colorPaletteGreenForeground1 } }}
                    />
                    <Stack>
                        <Text variant="xxLarge" styles={{ root: { fontWeight: 600 } }}>
                            Individual Chats
                        ...
                        <Text variant="large" styles={{ root: { color: tokens.colorNeutralForeground4, marginTop: 8 } }}>
                            Manage 1-on-1 customer conversations and direct messages
                        </Text>
                    </Stack>
                </Stack>
            </Stack>
