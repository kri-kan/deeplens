...
export function ColorAssignmentPickerModal({
  ...
  // Slot limits derived directly from swatch template
  const slotLimits = useMemo(() => getSlotLimits(template), [template]);

  const slotsScrollRef = useRef<ScrollView>(null);
  const slotsScrollOffsetRef = useRef<number>(0);

  // Dynamic slot colors state supporting 1, 2, 3, 4 slots strictly within template limits
  ...
  const handleAddSlot = () => {
    if (slotLimits.fixed || slotColors.length >= slotLimits.max) return;
  ...
  const handleRemoveSlot = (indexToRemove: number) => {
    if (slotColors.length <= slotLimits.min) return;
    const newColors = slotColors.filter((_, idx) => idx !== indexToRemove);
    setSlotColors(newColors);
    setActiveSlotIndex((prev) => Math.min(prev, newColors.length - 1));

    // When slot is removed, glide compressed slots towards left if scrolled right
    if (slotsScrollOffsetRef.current > 0) {
      slotsScrollRef.current?.scrollTo({ x: 0, animated: true });
      slotsScrollOffsetRef.current = 0;
    }
  };
      ...
      <View style={styles.sheetBackdrop}>
          ...
          <XStack alignItems="center" gap={10} flex={1}>
            ...
            <YStack flex={1} marginRight={6}>
              ...
              <Text fontSize={9.5} color={tokens.textMuted}>
                {slotLimits.fixed
                  ? `${slotColors.length} Color Slot${slotColors.length === 1 ? '' : 's'} (Fixed) · ${slotLimits.name.toUpperCase()}`
                  : `${slotColors.length} Color Slots (Limit: ${slotLimits.min}–${slotLimits.max}) · ${slotLimits.name.toUpperCase()}`}
              </Text>
            </YStack>
          </XStack>
        ...
        <View style={styles.slotsBarWrapper}>
          <ScrollView
            ...
            onScroll={(e) => {
              slotsScrollOffsetRef.current = e.nativeEvent.contentOffset.x;
            }}
          ...
          >
            {slotColors.map((hex, idx) => {
              ...
              let slotRoleName = `Slot ${slotLetter}`;
              let slotDetail = '';
              if (template === 'solid') {
                slotRoleName = 'Primary';
                slotDetail = '100%';
              } else if (template === 'contrast-border') {
                slotRoleName = idx === 0 ? 'Body' : 'Border';
                slotDetail = idx === 0 ? '80%' : '20%';
              } else if (template === 'multi-tone' || template === 'dual-tone') {
                slotRoleName = `Stop ${idx + 1}`;
                slotDetail = `${Math.round((idx / Math.max(slotColors.length - 1, 1)) * 100)}%`;
              } else if (template === 'multi-shade' || template === 'half-and-half') {
                slotRoleName =
                  slotColors.length === 2
                    ? idx === 0 ? 'Left' : 'Right'
                    : slotColors.length === 3
              ...
              return (
                ...
                >
                  ...
                  {slotDetail ? (
                    <Text fontSize={8} color={tokens.textMuted} fontWeight="600">
                      {slotDetail}
                    </Text>
                  ) : null}

                  {/* Remove Slot Action if above minimum slot limit */}
                  {slotColors.length > slotLimits.min && (
                    <Pressable
                      onPress={(e) => {
                        e.stopPropagation?.();
            ...
            {/* Square Add Slot Tile: ONLY when template is NOT fixed and slotColors is under max limit */}
            {!slotLimits.fixed && slotColors.length < slotLimits.max && (
